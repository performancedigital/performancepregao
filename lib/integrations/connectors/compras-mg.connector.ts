...
const BASE_URL = 'https://compras.mg.gov.br/api/v1'
const PAGE_SIZE = 50

interface MGLicitacao {
  id: number | string
  numero?: string
  objeto?: string
  modalidade?: string
...
function getToken(): string {
  return process.env.COMPRAS_MG_TOKEN || ''
}
...
export class ComprasMgConnector implements IConnector {
  ...
  async fetchIncremental(
    ...
    if (!token) {
      console.error('Compras MG: Token nao configurado (COMPRAS_MG_TOKEN)')
    ...
    while (hasMore && pagina <= 20) {
      try {
        const url = `${BASE_URL}/processos?data_inicio=${dataInicio}&data_fim=${dataFim}&pagina=${pagina}&limite=${PAGE_SIZE}`
        ...
        if (!res.ok) {
          ...
          if (res.status === 401) {
            console.error('Compras MG: Token invalido ou expirado')
            break
          }
          console.error(`Compras MG pag ${pagina}: HTTP ${res.status}`)
          break
        }

        const data = await res.json()
        const items: MGLicitacao[] = data.processos || data.licitacoes || data.data || data.content || []
        ...
        console.error(`Compras MG erro pag ${pagina}:`, err)
        break
      }
    }
  ...
  normalize(record: unknown): NormalizedBidding | null {
    const lic = record as MGLicitacao
    ...
    return {
      externalId: `MG-${lic.id}`,
      portalCode: 'COMPRAS_MG',
  ...
  async healthCheck(): Promise<ConnectorHealth> {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/processos?data_inicio=${today}&data_fim=${today}&pagina=1&limite=1`,
        {
          headers: {
            'Accept': 'application/json',
            'Authorization': `Bearer ${token}`,
